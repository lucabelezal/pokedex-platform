...
```text
core/infra/postgres/source-json/
├── 01_region.json
├── 02_type.json
├── 03_egg_group.json
├── 04_generation.json
...
1. Edite os arquivos em `core/infra/postgres/source-json`.
2. Rode o gerador JSON -> SQL.
3. Gere o seed em `core/infra/postgres/seeds/init-data.sql`.
...
- `core/infra/postgres/source-json/.local-tools/fetch_gen1_facts.py`

Execucao:

```bash
python3 core/infra/postgres/source-json/.local-tools/fetch_gen1_facts.py \
	--start 1 \
	--end 151 \
	--output-dir core/infra/postgres/source-json/generated
```
